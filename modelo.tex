...
\begin{itemize}
    \item Cálculo da admitância
    \item Cálculo da admitância efetiva
    \item Cálculo do coeficiente de reflexão
    \item Cálculo da pressão média
    \item Cálculo das ondas de pressão e fluxo
\end{itemize}

Como visto na equação~\eqref{eq:amitancia} para se obter o valor da admitância se faz necessário o valor da onda de pressão. Já o cálculo da admitância efetiva e do coeficiente de reflexão exigem também valores referentes à outras artérias o que se mostrou um desafio computacionalmente.Nas equações~\eqref{27_R} e~\eqref{28_Ye} é possível verificar a necessidade de valores de segmentos de níveis inferiores para se obter o valor atual.
Entretanto, o caminho oposto é necessário para se resolver o valor da pressão média de cada segmento como visto na equação~\eqref{18_barp}.
Com estes desafios em mente, a estrutura de dados desenvolvida na linguagem C++ utiliza de ponteiros para acessar os valores de artérias adjacentes. O algoritmo desenvolvido se dividiu em duas fases, em um primeiro momento realiza o cálculo dos segmentos finais ao segmento raiz e ,finalmente, da raiz aos segmentos finais.

\begin{algorithm}[H]
	\SetAlgoLined
	\Entrada{r(cm),L(cm),$\rho$(g/$cm^3$),E(g/$cm*s^2$),f(Hz),$\epsilon$,$\mu_0$,$\phi_0$ }
	\Inicio{
		Segmento a = raiz \textit{(começa a recursão pela raiz)}
		
		\Se{existe(a)}{
			Envia a recursão \textit{fase um}(a,f,$\mu_0$,$\phi_0$) e armazena $Y_r$
		}
		\Se{existe($Y_r$)}{
			Envia a recursão \textit{fase dois}(a,$p_0$,$Y_r$)
		}
	}
	\textbf{\textit{\textsl{\underline{fase um}}}}(a,f,$\epsilon$,$\phi_0$)
	\Inicio{
		...
		\Senao{
			Calcula $\alpha$,$\epsilon$,$E_v$,$c_v$,$Y_v$
		}
		\Se{a $\rightarrow$ folha}{
			$Y_e = Y$ e $R = 0$
		}
		\Senao{
			Calcula $Y_e$ e $R$
	...
	\textbf{\textit{\textsl{\underline{fase dois}}}}(a,$p_0$,$Y_r$)
	\Inicio{
	}
